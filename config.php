...
// connect to database
$conn = new mysqli("localhost", "root", "", "bethelex");
...
define('BASE_URL', 'http://localhost/bethelexhealthcare/'); // the home url 
...
{
    global $conn;
    $stmt = $conn->prepare($sql); //We are using prepared statements and this is important for security reasons.
    $stmt->bind_param($types, ...$params);
    $stmt->execute();
    $result = $stmt->get_result();
