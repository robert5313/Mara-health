...
// $cnt = 1;
	if ($query1->rowCount() > 0) {
		foreach ($results1 as $result1){
			if($result1->paid_amount){
				$previous_amount = $result1->paid_amount;
				$paid_amount=$_POST['paid_amount']+$previous_amount;
				$payment_code = uniqid('', true);
				$sql2="UPDATE booking set paid_amount=:paid_amount, payment_code=:payment_code where bid=:bid";
				$query2 = $dbh->prepare($sql2);
				$query2->bindParam(':paid_amount',$paid_amount,PDO::PARAM_STR);
				$query2->bindParam(':payment_code',$payment_code,PDO::PARAM_STR);
